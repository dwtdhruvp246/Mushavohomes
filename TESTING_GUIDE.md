...
- An authorised landlord/manager creates a move-in inspection through `save_property_inspection` without sending ownership IDs.
- Room conditions and notes persist, evidence uses `landlord_id/inspection_id/file`, and the record remains visible after refresh.
- The linked tenant sees completed or locked inspections resolved from their authenticated profile and lease.
- The linked tenant can sign an unsigned completed inspection through `tenant_sign_inspection`.
- A different tenant cannot select the inspection, open its evidence, or sign it even when UUIDs or paths are changed manually.
- Draft inspections are management-only; locked inspections cannot be edited, receive new evidence, or be signed.
